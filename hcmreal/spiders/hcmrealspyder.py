...
class ElectronicsSpider(CrawlSpider):
    ...
    start_urls = [
        'http://www.muabannhadat.vn/can-ho-ban-chung-cu-3529/tp-ho-chi-minh-s59',
    ]
    ...
    def parse_detail_page(self, response):
        ...
        try:
            ...
            print('error while getting talbes')
            
        try:
            if 'price' in realItem and 'area' in realItem and 'bedrooms' in realItem:
                if len(realItem['price']) > 0 and len(realItem['area']) > 0 and len(realItem['bedrooms'])>0:
                    yield realItem
        except:
            print("error")
